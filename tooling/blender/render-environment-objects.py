...
def main() -> None:
    ...
    for collection in sorted(bpy.data.collections, key=lambda item: item.name):
        ...
        # Repeating floor and wall tiles must meet at pixel edges.
        margin_fraction = 0.0 if asset_id in {"floor.linoleum.institutional", "wall.interior.cap.overhead", "wall.interior.face", "terrain.dirt.compacted"} else MARGIN_FRACTION
