...
def main() -> None:
    ...
    for collection in sorted(bpy.data.collections, key=lambda item: item.name):
        ...
        margin_fraction = 0.0 if asset_id in {"floor.linoleum.institutional", "wall.interior.cap.overhead", "wall.interior.face", "door.interior.face", "terrain.dirt.compacted", "terrain.grass.mown", "terrain.concrete.paving"} else MARGIN_FRACTION
