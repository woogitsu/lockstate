...
def main() -> None:
    ...
    for collection in sorted(bpy.data.collections, key=lambda item: item.name):
        ...
        margin_fraction = 0.0 if asset_id in {"floor.linoleum.institutional", "floor.kitchen.nonslip", "floor.canteen.terrazzo", "floor.yard.compacted-earth", "floor.shower.ceramic", "floor.laundry.nonslip", "floor.infirmary.vinyl", "floor.common-room.cork-rubber", "floor.classroom.oak-laminate", "floor.security-office.antistatic", "floor.cell.sealed-concrete", "floor.staff-room.woven-vinyl", "wall.interior.cap.overhead", "wall.interior.face", "door.interior.face", "terrain.dirt.compacted", "terrain.grass.mown", "terrain.concrete.paving", "terrain.gravel.service_path", "terrain.rock.bedrock", "terrain.water.still"} else MARGIN_FRACTION
