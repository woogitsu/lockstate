...
def main() -> None:
    ...
    for collection in sorted(bpy.data.collections, key=lambda item: item.name):
        ...
        margin_fraction = 0.0 if asset_id in {"floor.linoleum.institutional", "floor.kitchen.nonslip", "floor.canteen.terrazzo", "floor.yard.compacted-earth", "floor.shower.ceramic", "floor.laundry.nonslip", "wall.interior.cap.overhead", "wall.interior.face", "door.interior.face", "terrain.dirt.compacted", "terrain.grass.mown", "terrain.concrete.paving", "terrain.gravel.service_path", "terrain.rock.bedrock"} else MARGIN_FRACTION
