...
const rows: string[] = [];
rows.push('seed | assaults | riots | escapeAttempts | gangRetaliations | gangMembers | grudges | maxRiskTier');
for (const seed of SEEDS) {
  ...
  rows.push(`0x${seed.toString(16)} | ${String(assaults)} | ${String(riots)} | ${String(escapes)} | ${String(retaliations)} | ${String(members)} | ${String(grudges)} | ${String(maxTier)}`);
}
