 ...
 *
 * Measuring settled it: 240 KiB is right for the claim both sentences make (an
 * empty prison, every array at its constructor default), and ~300 KiB was a
 * populated prison's figure — which actually measures ~337 KiB — stated as if
 * it were the same case.
 *
...
describe('the capacity-shaped payload figure the documentation quotes', () => {
  it('still has the three inputs the figure is derived from', () => {
    ...
    expect(NEED_MAX, "needs default to NEED_MAX, and its digit width is part of the figure").toBe(255);
  });

  it('measures 240 KiB for an empty prison at capacity, which is the claim both documents make', () => {
    const bytes = encodedByteSizeAtCapacity(DEFAULT_PRISONER_CAPACITY, (name) =>
      name === 'needs' ? NEED_MAX : name === 'actionIndex' ? -1 : 0,
    );

    expect(bytes).toBe(245_332);
    expect(Math.round(bytes / 1024), 'both documents say ~240 KiB').toBe(240);
  });
  ...
  it('measures more for a populated prison, which is where the ~300 KiB figure came from', () => {
    ...
    expect(Math.round(bytes / 1024)).toBe(337);
    ...
    expect(bytes).toBeGreaterThan(245_332);
  });
});
