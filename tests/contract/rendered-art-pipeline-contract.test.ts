 ...
 * fields -- `footprintTiles` and `sizePx` -- for every one of the 36 entries,
 ...
 * adds the 29th; medical bed, medicine cabinet and stove add three; washer, security console, utility panel and loading dock gate bring the total to 36. It runs with no Blender,
...
describe('environment render aspect invariant (recomputed, not trusted)', () => {
  it('exactly reproduces the footprint aspect, for every one of the 37 rendered entries', async () => {

    const sidecarPath = resolve(root, 'assets/rendered/environment/environment-objects.render.json');
    const sidecar = JSON.parse(await readFile(sidecarPath, 'utf8')) as {
      entries: Array<{ assetId: string; footprintTiles: { width: number; height: number }; sizePx: { width: number; height: number } }>;
    };
    expect(sidecar.entries.length).toBe(37);
