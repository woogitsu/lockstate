import type { SimulationEnumForm, SimulationEnumGroup } from '../../src/content/simulation-message-keys';
import { stripComments } from './canonical-iteration';

/**
 * Reads the enumerations the simulation declares straight out of its source
 * text, so `src/content/simulation-message-keys.ts` can be held to them.
 *
 ...
 * the rules in an exported module rather than inside the test means they are
 * exported, type-checked and unit-testable against fixtures, instead of being
 * regexes that only ever run against real files and can therefore quietly
 * stop matching.
 *
 ...
 *
 * ## Why this lives under `tests/helpers/` and not in `src/` (#307)
 *
 * It used to be the back half of `src/content/validate-catalog.ts`, which
 * kept it beside a runtime cross-catalog check and made it, on paper, part
 * of the shipped client. It carried its own comment stripper for that reason:
 * a two-regex `stripSourceComments` that removed block comments and then line
 * comments, which is exactly the construction #278 found wrong in
 * `stripComments` and #298 replaced. Worse here, because it collapsed a block
 * comment to a single space instead of preserving newlines, so every offset
 * and line number derived from its output was wrong as well. Measured over
 * the 141 files this scan reads, it blanked **81 lines of real code in two of
 * them** -- 80 consecutive locale lines of `src/content/default-locale-en.ts`,
 * where the `//` comment naming `src/persistence/**` opened a block comment
 * that closed 173 lines later, and the line of `validate-catalog.ts` that
 * declared the stripper itself.
 *
 * Sharing #298's single-pass scanner was the obvious fix and appeared to cost
 * a several-hundred-line lexer in the client bundle. It does not, because
 * nothing in the client reaches this code: the enum-scanning exports had no
 * importer anywhere in `src/`, and the module that held them was reached only
 * through `src/content/index.ts` <- `src/simulation/rooms/definition.ts`,
 * whose sole importer is a test. So the tradeoff was not paid, it was
 * removed: the scanning rules moved here, beside the scanner they now share,
 * and `src/content/validate-catalog.ts` keeps only the runtime cross-catalog
 * check that genuinely belongs to the shipped catalogs.
 *
 * Comments are still stripped before scanning, so a rule can be *discussed*
 * in prose without the prose reading as a declaration -- but now through
 * `stripComments`, the one implementation
 * `tests/foundation/comment-stripping-contract.test.ts` pins in both
 * directions over the whole repository.
 */
...
export function validateSimulationEnumGroupSource(
  ...
  const extraction = extractEnumDeclarationIds(stripComments(source), group.declaration, group.form);
  if (!extraction.ok) {
    return [
      {
