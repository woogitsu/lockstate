...
test.describe('the assembled application', () => {
  ...
  test('the Build panel buys materials, and a real PurchaseMaterials reaches the worker (#89)', async ({ page }) => {
     ...
     *
     * This is the half that reaches the player on *this* side of the
     * dispatch. `Treasury.spend` refuses rather than overdrawing, and since
     * #261 the worker no longer drops that outcome: `session-commands.ts`
     * records it in the session's `RefusalLog` and it arrives as an alert
     * row. What the guard in `src/main.ts` adds is the case a player can
     * actually provoke -- asking for more than the prison has ever had --
     * reported on the refusal line, on the button that was pressed, at every
     * viewport, where `hud.css` drops the whole alerts region at 720px and
     * below. Without it the player would press Buy, watch the money not move,
     * and be told nothing they could see on a phone: the failure #82 and #207
     * exist for. The guard reads the balance the worker published, which is
     * why it needs a real session and a real page.
     *
     * The two are exclusive per press, which the last assertion below
     * measures: the throw happens instead of the submit, so a purchase
     * refused here never became a command and the worker has nothing to
     * report about it.
     *
