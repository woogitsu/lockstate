...
describe('cancelling a purchase whose delivery has not landed', () => {
  ...
  it('buys, cancels, and returns the balance to exactly what it was', () => {
    ...
    expect(runtime.treasury.balanceMinorUnits, 'a purchase spends at the tick it executes').toBe(24_880);
    ...
    expect(runtime.treasury.balanceMinorUnits, 'the refund must be exact').toBe(25_000);
  ...
  it('cancels the delivery that was named and leaves the others paid for', () => {
    ...
    expect(runtime.treasury.balanceMinorUnits).toBe(24_720);

    send(runtime, 'cmd-cancel-b', { type: 'CancelMaterialPurchase', orderId: 'buy-b' });

    expect(runtime.treasury.balanceMinorUnits, 'only the named purchase is refunded').toBe(24_800);
    ...
    expect(runtime.treasury.balanceMinorUnits).toBe(24_800);
  });
  ...
  it('refuses a cancellation whose delivery has already arrived, and creates no money', () => {
    ...
    expect(runtime.treasury.balanceMinorUnits, 'a delivered purchase must not be refunded').toBe(24_920);
  ...
  it('cancels a purchase that survived a snapshot, because the pending list is saved state', () => {
    ...
    expect(restored.treasury.balanceMinorUnits, 'a restored delivery refunds the price it was bought at').toBe(25_000);
