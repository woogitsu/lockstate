...
import { createSaveEnvelope, decodeSaveEnvelope, type SaveEnvelopeV6 } from '../../src/persistence/save-schema';
...
describe('a V6 walk survives the V7 intake migration (#1373, #594)', () => {
  function v6Snapshot(runtime: SimulationRuntime): SessionSnapshotBundle {
    const bundle = captureSessionSnapshot(runtime);
    if (bundle.simulation === undefined) throw new Error('the session has no simulation section');
    const {
  ...
  it('decodes a genuine V6-shaped mid-walk save, restores the walk, and continues exactly', { timeout: 120_000 }, () => {
    const continuous = buildPrison(PRISONERS_ONLY);
    stepTo(continuous, DAY + 1);
    while (walkingPrisoners(continuous) === 0 || continuous.prisoners.locomotion.walkingCount === 0) {
      if (continuous.kernel.tick > 2 * DAY) throw new Error('no prisoner walked during the day');
      continuous.kernel.step();
    }
    const atSave = v6Snapshot(continuous);
    expect(atSave.simulation?.inFlight?.prisoners.locomotion.walks.length).toBeGreaterThan(0);
    const payload = JSON.parse(JSON.stringify(atSave)) as SessionSnapshotBundle;
    const old: SaveEnvelopeV6 = {
      saveSchemaVersion: 6,
      gameVersion: 'lockstate-0.0.0',
      prisonId: 'v6-mid-walk',
      revision: 1,
      createdAt: 1,
      updatedAt: 2,
    ...
    const decoded = decodeSaveEnvelope(JSON.parse(JSON.stringify(old)) as unknown);
    expect(decoded.ok).toBe(true);
    if (!decoded.ok) throw new Error(`V6 save rejected: ${decoded.error.code}`);
    expect(decoded.migrated).toBe(true);
    expect(decoded.value.payload.simulation?.inFlight).toEqual(atSave.simulation?.inFlight);
    const restored = restoreSimulationRuntime(decoded.value.payload as unknown as SessionSnapshotBundle, SEED).runtime;
    expect(walkingPrisoners(restored)).toBe(walkingPrisoners(continuous));
    expect(restored.prisoners.locomotion.walkingCount).toBe(continuous.prisoners.locomotion.walkingCount);
    expect(firstDifferences(atSave, v6Snapshot(restored))).toEqual([]);

    const nextTick = continuous.kernel.tick + 50;
    stepTo(continuous, nextTick);
    stepTo(restored, nextTick);
    expect(firstDifferences(v6Snapshot(continuous), v6Snapshot(restored))).toEqual([]);
    const migratedCandidates = captureSessionSnapshot(restored).simulation?.intakeCandidates;
    expect(migratedCandidates).toBeDefined();
    expect(saveAndRestore(restored).intakeCandidates.snapshot()).toEqual(migratedCandidates);
  });
});

/**
 * **This fixture now converges, but the budget risk remains open.** The
 * original #1373 branch measured a different served set for these 24 actors
 * after restore. On the combined #594/#1373 branch, five requests still defer
 * at the same boundary, yet the restored and continuous snapshots match at
 * both ticks 3462 and 3463. The old test explicitly required inversion when
 * that happened, so this now requires equality at both boundaries.
 *
...
describe('a save taken before the navigation budget binds (#1373)', () => {
  it('24 prisoners at the tick-3461 block change still continue exactly', { timeout: 120_000 }, () => {
    ...
    const continuousAtFirstTick = captured(continuous);
    ...
    stepTo(restored, binds + 1);
    expect(restored.navigation.getInFlightSnapshot().pending.length).toBe(deferred.length);
    expect(captured(restored)).toBe(continuousAtFirstTick);
    stepTo(restored, binds + 2);
    expect(captured(restored)).toBe(captured(continuous));
  });
});
