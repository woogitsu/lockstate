...
describe('blocked deploy announcement contract', () => {
  ...
  it('needs no credential and publishes nothing', async () => {
    ...
    // The owner moved every job to GitHub-hosted Ubuntu before making the
    // repository public. The same selector is checked across workflows by
    // ci-configuration-contract.test.ts.
    expect(
      body,
      `\`staging-blocked\` in ${DEPLOY} no longer runs on the GitHub-hosted Ubuntu runner.`,
