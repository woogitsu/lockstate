...
const AWAITING_CONSUMER: Readonly<Record<string, string>> = {
  'selection.primary':
    'Emitted by `PointerInputAdapter` when the `world` context is active, and read by nobody: the returned event array reaches `world-scene.ts` and is discarded. No key is bound to it either, so no player can press it and find nothing happening. **The clause this entry used to rest on is spent as of issue #895 and is quoted rather than deleted, because what it was protecting still holds:** it read "Nothing in the repository selects anything yet -- there is no selection state, no highlight and no inspector -- so unlike the zoom keys this one is not a dead path to a working behaviour but a declared control for a feature that does not exist (#141, #200)." There is a selection now, and an inspector with it -- the Regime panel\'s roster rows carry `role="radio"` and `aria-checked`, a press raises the `select-prisoner` HUD intent, and `src/ui/simulation-prisoner-detail.ts` reads `hud/prisoner-detail` for the chosen prisoner. None of that is *this* action: that selection is a HUD row press, keyed on an `EntityId` the roster projection published, and it neither reads nor needs a **world-pointer** hit against a rendered actor -- there is still no highlight in `src/rendering/` and nothing anywhere maps a world position to an entity. So this stays a declared control for a feature that does not exist, and the feature it waits on is now specifically world-pointer selection rather than selection at all.',
  'build.confirm':
