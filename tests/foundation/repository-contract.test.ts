...
describe('repository foundation contract', () => {
  ...
  it('lists nothing in tsconfig include that is not on disk', async () => {
    /*
     * TypeScript silently ignores an entry in `include` that resolves to
     * nothing. `tsc -b` stays clean, so the entry is invisible -- and an
     * invisible entry is not inert: it reads as "this file is typechecked"
     * to anyone who opens the file.
     *
     * That is not hypothetical here. `tsconfig.json` listed
     * `worker-configuration.d.ts` for as long as it did precisely because
     * nothing could see it: the file did not exist, was generated by
     * nothing, and was not gitignored, and the entry survived every gate in
     * this directory (#141). It has since been removed. Nothing stopped it
     * coming back, which is what this closes.
     *
     * A glob is accepted without checking, because a glob legitimately
     * matches nothing today and something tomorrow. Every entry in this
     * repository's `include` is a plain path, and the assertion below says
     * so -- so a glob appearing here is a visible change to this test rather
     * than a silent widening of what goes unchecked.
     */
    const tsconfig = await readRepositoryJson<TypeScriptContract>('tsconfig.json');

    // Vacuity guard: an empty or misparsed `include` would make the loop
    // below assert nothing at all.
    expect(tsconfig.include.length).toBeGreaterThanOrEqual(4);

    const globbed = tsconfig.include.filter((entry) => /[*?[\]]/u.test(entry));
    expect(
      globbed,
      'tsconfig include gained a glob. That is allowed, but this check cannot verify one -- decide what should happen and say so here',
    ).toEqual([]);

    const missing: string[] = [];
    for (const entry of tsconfig.include) {
      try {
        await stat(path.join(repositoryRoot, entry));
      } catch {
        missing.push(entry);
      }
    }
    expect(
      missing,
      'tsconfig include names a path that does not exist. TypeScript ignores it without a word, so the entry looks like coverage and is not: create the file, generate it, or remove the entry',
    ).toEqual([]);
  });
});
