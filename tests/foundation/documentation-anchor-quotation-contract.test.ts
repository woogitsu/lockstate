 ...
 * be repaired:
 *
 * - **`docs/adr/STATUS-QUEUE.md`, 300 -> 302**, and it is a raise for exactly
 *   the reason the `283` entry above gives: one anchor cited twice and
 *   therefore counted twice, `src/content/locale-pl.ts:803`
 *   (`Zlecenie ukończono.`), slid to `:847` when the branch inserted thirty-one Polish
 *   strings above it, and **this branch must not write that file** --
 *   [#1321](https://github.com/woogitsu/lockstate/pull/1321) is a re-anchor
 *   pass in it and open. The repair belongs to whoever writes that file next;
 *   raising the row here buys nothing but the ordering, and the prediction
 *   made at `283` that such a repair would follow is recorded above as having
 *   already failed once.
