 ...
 * reach it. `HudIntent` now declares eight members, two of which are `undo` and
 ...
 *
 * `HireStaff` is the seventh command and arrived with its producer, which is
 * the only entry here that never spent a day on the list below. Its consumer
 * was in the same position `PurchaseMaterials`'s had been in, one layer down:
 * `GuardRoster.hire` was complete, snapshotted and restored, and **every call
 * in the repository was in a test**, so the four systems that read the roster
 * -- deployment, patrol, incident response and contraband search -- iterated
 * an empty collection in every session a player could start. ADR 0025 records
 * the surface that closed it, and `src/main.ts` dispatches the command from
 * the Staff panel's one button.
 *
 ...
 *   below fails loudly if any of the five that do exist stops being found.
 ...
 * eighth member is in scope the moment it is added. A hand-written array
...
describe('every declared simulation command either has a producer or is accounted for', () => {
  it('scans a real source set and really finds the producer that exists', () => {
    ...
    // where it is -- all five in `src/main.ts`, which is the composition root
  ...
  it('measures five produced and two unproduced, which is where ADR 0025 left the command surface', () => {
    ...
    // in, and two and five once ADR 0025 added `HireStaff` *with* its
    // producer -- the one command that has never been on the list above. Both
    ...
    // stay green. Note the denominator moved too, so a seventh command added
