...
4. [`docs/adr/`](./docs/adr/) — architectural decision records. Read the `Status` line of any ADR before treating it as settled. **The sentence that used to stand here — "Every ADR on disk is `Accepted`", with 0029 named as the last one that had been `Proposed` — is false and is corrected rather than deleted, because it is the kind of tally `docs/AGENT_WORKFLOW.md` §4 says rots first and it rotted the next morning.** It was true on 2026-08-26; `c78908b` landed ADR 0042 `Proposed` at 09:12 the following day and nothing here moved for eighteen days. **A tally stood here and is deleted rather than corrected, because it is the demonstration.** It read *"Swept 2026-09-13, reading each ADR's own `Status`: 105 ADRs on disk, 62 `Accepted` and 43 `Proposed` — the 43rd being ADR 0112"*, and it closed by claiming that it "now names no number that a new ADR would change" **in the same sentence in which it named three**. Re-derived on 2026-09-15 by replicating `tests/foundation/adr-status-reference-contract.test.ts`'s own `statusStatement` extractor over `docs/adr/`: **109, 65 and 44** — every one of the three wrong, in two days — and ADR 0112 is `Accepted`, not the 43rd `Proposed`, because the owner accepted it on the day that sentence was typed. No number replaces them here: `docs/adr/README.md`'s own table is the record, and this line is not. [`docs/adr/STATUS-QUEUE.md`](./docs/adr/STATUS-QUEUE.md) §2 is the owner's decision queue, and the sections after it record where an accepted decision and the code still disagree.
5. [`docs/VISUAL_IDENTITY.md`](./docs/VISUAL_IDENTITY.md) — the visual identity, product voice and twenty-article constitution the owner delivered on 2026-09-13, with the measured gap against today's interface. The delivery itself is vendored verbatim under [`docs/design/`](./docs/design/) and is never edited; [`docs/IDENTITY_V5_ROLLOUT.md`](./docs/IDENTITY_V5_ROLLOUT.md) is the nine-stage plan and [ADR 0112](./docs/adr/0112-what-the-2026-09-13-identity-delivery-decides.md) is the decision record. **Read it before changing anything under `src/ui/`.**
