...
| `hud.alert.refusal.purchase.delivery-capacity` | The materials were not ordered — there is not enough storage space for them. Use stock, cancel a delivery, or add storage racks. | `src/content/default-locale-en.ts:1266` |
...
| `hud.alert.refusal.zone.not-enclosed` | The room was not zoned — finished walls or doors must line the tile edges around it; the outline still has a gap. | `src/content/default-locale-en.ts:1400` |
...
| `hud.build.note` | Press Play; clock builds orders. | `src/content/default-locale-en.ts:2360` |
...
| `hud.security.coverage-no-posts` | No posts | `src/content/default-locale-en.ts:3067` |
| `hud.security.coverage-no-posts-hint` | No guard posts are required right now. | `src/content/default-locale-en.ts:3068` |
| `hud.security.coverage-met` | Covered | `src/content/default-locale-en.ts:3069` |
| `hud.security.coverage-no-reserve` | No reserve | `src/content/default-locale-en.ts:3070` |
| `hud.security.coverage-no-reserve-hint` | Hire {count} for the largest response. | `src/content/default-locale-en.ts:3071` |
| `hud.security.coverage-no-reserve-consequence` | No free guards for incidents or searches. | `src/content/default-locale-en.ts:3072` |
| `hud.security.coverage-met-hint` | Incidents and searches need free guards. | `src/content/default-locale-en.ts:3245` |
| `hud.security.coverage-short` | Understaffed | `src/content/default-locale-en.ts:3246` |
| `hud.security.coverage-short-hint` | Hire {count} more to cover this population. | `src/content/default-locale-en.ts:3247` |
| `hud.security.coverage-unguarded` | Unguarded | `src/content/default-locale-en.ts:3248` |
| `hud.security.coverage-unguarded-hint` | Nobody is on duty. Hire {count} to cover this population. | `src/content/default-locale-en.ts:3249` |
| `hud.security.post-unreachable` | Post cut off | `src/content/default-locale-en.ts:3278` |
| `hud.security.post-unreachable-hint` | No guard can reach the post, so nobody is on duty. Taking down a wall beside it opens the way back. | `src/content/default-locale-en.ts:3279` |
| `hud.security.coverage-overcrowded` | Overcrowded | `src/content/default-locale-en.ts:3309` |
| `hud.security.coverage-overcrowded-hint` | More prisoners than beds: every prisoner's safety runs down faster, and past a point their hygiene does too, until there is a bed for each of them. | `src/content/default-locale-en.ts:3310` |
| `hud.security.coverage-unguarded-consequence` | No guard is posted here, so nobody in this sector is kept safe. | `src/content/default-locale-en.ts:3359` |
| `hud.regime.title` | Regime | `src/content/default-locale-en.ts:3375` |
| `hud.regime.blocks` | Today's blocks | `src/content/default-locale-en.ts:3376` |
| `hud.regime.block-allows` | Allows {categories} | `src/content/default-locale-en.ts:3377` |
| `hud.regime.block-progress` | {percent}% through | `src/content/default-locale-en.ts:3378` |
| `hud.regime.category-separator` | ,  | `src/content/default-locale-en.ts:3381` |
| `hud.regime.edit` | Change the block running now | `src/content/default-locale-en.ts:3399` |
| `hud.regime.edit-last-category` | A block has to allow at least one thing, so the last one cannot be switched off. | `src/content/default-locale-en.ts:3408` |
| `hud.regime.sentence-remaining` | Sentence remaining (in-game days): {days} | `src/content/default-locale-en.ts:3411` |
| `hud.regime.roster` | Prisoners | `src/content/default-locale-en.ts:3424` |
| `hud.regime.roster-count` | {shown} of {total} | `src/content/default-locale-en.ts:3425` |
| `hud.regime.roster-name` | {given} {family} | `src/content/default-locale-en.ts:3426` |
| `hud.regime.roster-unnamed` | Prisoner {id} | `src/content/default-locale-en.ts:3427` |
| `hud.regime.roster-heading` | Heading to {activity} | `src/content/default-locale-en.ts:3428` |
| `hud.regime.roster-more` | and {count} more | `src/content/default-locale-en.ts:3429` |
| `hud.regime.roster-empty` | No prisoners yet. Build a cell — big enough, walled all round, with a bed and a toilet in it — to take somebody in. | `src/content/default-locale-en.ts:3461` |
| `hud.regime.risk-tier-explanation` | {standing} risk tier. Classification uses sentence length, prior incidents and conduct, and is reviewed over time. | `src/content/default-locale-en.ts:3464` |
| `hud.regime.intake-standing-explanation` | {standing} intake status. A risk tier has not been assigned yet. | `src/content/default-locale-en.ts:3465` |
| `hud.regime.roster-emptied` | This prison is empty. Take somebody in to start again. | `src/content/default-locale-en.ts:3510` |
| `hud.regime.roster-open-tab` | Open {tab} | `src/content/default-locale-en.ts:3511` |
| `hud.refusal.set-clock` | The clock did not change — the request was refused. | `src/content/default-locale-en.ts:3526` |
| `hud.refusal.place-build-order` | The build order was not placed — the request was refused. | `src/content/default-locale-en.ts:3527` |
| `hud.refusal.purchase-materials` | Nothing was bought — the purchase was refused and no money was spent. | `src/content/default-locale-en.ts:3528` |
| `hud.refusal.hire-staff` | Nobody was hired — the request was refused and no money was spent. | `src/content/default-locale-en.ts:3529` |
| `hud.refusal.purchase-materials-past-floor` | Nothing was bought — deliveries are refused until the prison earns the money. | `src/content/default-locale-en.ts:3552` |
| `hud.refusal.hire-staff-past-floor` | Nobody was hired — hiring is refused until the prison earns the money. | `src/content/default-locale-en.ts:3553` |
| `hud.refusal.undo` | Nothing was undone — the request was refused. | `src/content/default-locale-en.ts:3554` |
| `hud.refusal.redo` | Nothing was redone — the request was refused. | `src/content/default-locale-en.ts:3555` |
| `hud.refusal.zone-room` | The room was not designated — the request was refused. | `src/content/default-locale-en.ts:3556` |
| `hud.refusal.unzone-room` | Nothing was removed — the request was refused. | `src/content/default-locale-en.ts:3557` |
| `hud.refusal.admit-prisoner` | Nobody was admitted — the request was refused. | `src/content/default-locale-en.ts:3558` |
| `hud.refusal.admit-prisoner-no-room` | Nobody was admitted — this prison has no room to hold anybody. | `src/content/default-locale-en.ts:3574` |
| `hud.refusal.cancel-build-order` | The order is still queued — the request was refused. | `src/content/default-locale-en.ts:3575` |
| `hud.refusal.cancel-material-purchase` | Nothing was refunded — the request was refused and the delivery is still on its way. | `src/content/default-locale-en.ts:3581` |
| `hud.refusal.sell-materials` | Nothing was sold — the request was refused and nothing was taken from stock. | `src/content/default-locale-en.ts:3588` |
| `hud.refusal.release-guard` | Nobody was released — the request was refused and the guard is still assigned. | `src/content/default-locale-en.ts:3589` |
| `hud.rooms.title` | Rooms | `src/content/default-locale-en.ts:3591` |
| `hud.rooms.catalogue` | Room type and area | `src/content/default-locale-en.ts:3595` |
| `hud.rooms.catalogue-empty` | No room types are available | `src/content/default-locale-en.ts:3596` |
| `hud.rooms.selected` | Selected | `src/content/default-locale-en.ts:3597` |
| `hud.rooms.arm` | Draw on map | `src/content/default-locale-en.ts:3598` |
| `hud.rooms.disarm` | Stop drawing | `src/content/default-locale-en.ts:3599` |
| `hud.rooms.arm-hint` | Drag a rectangle across the tiles this room should cover. | `src/content/default-locale-en.ts:3600` |
| `hud.rooms.remove` | Remove rooms | `src/content/default-locale-en.ts:3601` |
| `hud.rooms.remove-active` | Stop removing | `src/content/default-locale-en.ts:3602` |
| `hud.rooms.remove-hint` | Drag across any part of a room to remove all of it. | `src/content/default-locale-en.ts:3611` |
| `hud.rooms.area` | Area | `src/content/default-locale-en.ts:3612` |
| `hud.rooms.area-none` | Nothing selected | `src/content/default-locale-en.ts:3613` |
| `hud.rooms.area-value` | {width} × {height} tiles at {x}, {y} | `src/content/default-locale-en.ts:3614` |
| `hud.rooms.confirm` | Designate {width} × {height} | `src/content/default-locale-en.ts:3615` |
| `hud.rooms.confirm-remove` | Remove {width} × {height} | `src/content/default-locale-en.ts:3618` |
| `hud.rooms.cancel` | Discard | `src/content/default-locale-en.ts:3619` |
| `hud.rooms.minimum` | Needs at least {width} × {height} tiles | `src/content/default-locale-en.ts:3620` |
| `hud.rooms.minimum-none` | No minimum size | `src/content/default-locale-en.ts:3621` |
| `hud.rooms.too-small` | Too small — this room needs at least {width} × {height} tiles. | `src/content/default-locale-en.ts:3622` |
| `hud.rooms.enclosure` | Enclosure | `src/content/default-locale-en.ts:3623` |
| `hud.rooms.enclosure-none` | Not evaluated yet | `src/content/default-locale-en.ts:3624` |
| `hud.rooms.enclosure-sealed` | Walled in — not a door check | `src/content/default-locale-en.ts:3667` |
| `hud.rooms.enclosure-open` | Open on at least one side | `src/content/default-locale-en.ts:3668` |
| `hud.rooms.requirement-enclosed` | Needs walls or doors all round | `src/content/default-locale-en.ts:3679` |
| `hud.rooms.requirement-outdoors` | Must be outdoors | `src/content/default-locale-en.ts:3680` |
| `hud.rooms.requirement-none` | No enclosure rule | `src/content/default-locale-en.ts:3681` |
| `hud.rooms.requires-object` | Needs {count} × {object} | `src/content/default-locale-en.ts:3688` |
| `hud.rooms.requires-none` | No objects needed | `src/content/default-locale-en.ts:3693` |
| `hud.rooms.coordinates` | Enter coordinates | `src/content/default-locale-en.ts:3697` |
| `hud.rooms.coordinates-hint` | The keyboard route. Dragging on the map is quicker. | `src/content/default-locale-en.ts:3698` |
| `hud.rooms.coordinates-submit` | Use these tiles | `src/content/default-locale-en.ts:3703` |
| `hud.rooms.tile-x` | Tile X | `src/content/default-locale-en.ts:3704` |
| `hud.rooms.tile-y` | Tile Y | `src/content/default-locale-en.ts:3705` |
| `hud.rooms.width` | Width | `src/content/default-locale-en.ts:3706` |
| `hud.rooms.height` | Height | `src/content/default-locale-en.ts:3707` |
| `hud.rooms.step-down` | Decrease {field} | `src/content/default-locale-en.ts:3708` |
| `hud.rooms.step-up` | Increase {field} | `src/content/default-locale-en.ts:3709` |
| `hud.rooms.needs` | Not ready | `src/content/default-locale-en.ts:3714` |
| `hud.rooms.needs-count` | {unfinished} of {total} | `src/content/default-locale-en.ts:3715` |
| `hud.rooms.needs-room` | {room} at {x}, {y} is missing | `src/content/default-locale-en.ts:3720` |
| `hud.rooms.needs-object` | {count} × {object} | `src/content/default-locale-en.ts:3724` |
| `hud.rooms.needs-object-uncounted` | {object} | `src/content/default-locale-en.ts:3729` |
| `hud.rooms.needs-item-more` | and {count} more | `src/content/default-locale-en.ts:3734` |
| `hud.rooms.needs-object-unknown` | something this build cannot name | `src/content/default-locale-en.ts:3738` |
| `hud.rooms.needs-doorway` | a door — nobody can get in | `src/content/default-locale-en.ts:3765` |
| `hud.rooms.needs-unreachable` | a way in — nothing outside can reach its door | `src/content/default-locale-en.ts:3807` |
| `hud.rooms.at-capacity` | At capacity | `src/content/default-locale-en.ts:3867` |
| `hud.rooms.at-capacity-count` | {full} of {total} | `src/content/default-locale-en.ts:3868` |
| `hud.rooms.at-capacity-room` | {room} at {x}, {y} is full | `src/content/default-locale-en.ts:3869` |
| `hud.rooms.at-capacity-places` | places in use: {inUse} of {capacity} | `src/content/default-locale-en.ts:3870` |
| `hud.security-section.title` | Security | `src/content/default-locale-en.ts:3892` |
| `hud.security-section.waiting` | No prison is reporting. | `src/content/default-locale-en.ts:3901` |
| `hud.security-section.sectors` | Sectors | `src/content/default-locale-en.ts:3904` |
| `hud.security-section.sectors-empty` | No sector has been drawn on this land yet. | `src/content/default-locale-en.ts:3911` |
| `hud.security-section.sector-staffing` | {assigned} of {required} guards assigned | `src/content/default-locale-en.ts:3918` |
| `hud.security-section.sector-short` | {count} short | `src/content/default-locale-en.ts:3925` |
| `hud.security-section.sector-open-incidents` | {count} open here | `src/content/default-locale-en.ts:3931` |
| `hud.security-section.lockdown` | Lockdown | `src/content/default-locale-en.ts:3940` |
| `hud.security-section.incidents` | Incidents | `src/content/default-locale-en.ts:3943` |
| `hud.security-section.incidents-none` | Nothing has been recorded yet. | `src/content/default-locale-en.ts:3952` |
| `hud.security-section.incidents-closed` | Nothing is open. {total} recorded so far. | `src/content/default-locale-en.ts:3961` |
| `hud.security-section.incidents-summary` | {open} open of {total} recorded | `src/content/default-locale-en.ts:3963` |
| `hud.security-section.incidents-toll` | {injured} hurt, {escapes} got out | `src/content/default-locale-en.ts:3970` |
| `hud.security-section.incident-row` | {type} in {sector} | `src/content/default-locale-en.ts:4008` |
| `hud.security-section.incident-severity` | Severity {severity} of {max} | `src/content/default-locale-en.ts:4015` |
| `hud.security-section.incident-people` | {count} taking part | `src/content/default-locale-en.ts:4021` |
| `hud.security-section.incident-timeline` | How it went | `src/content/default-locale-en.ts:4023` |
| `hud.security-section.incident-timeline-row` | {state} at tick {tick} | `src/content/default-locale-en.ts:4034` |
| `hud.security-section.incident-responders` | Responders needed: {count} | `src/content/default-locale-en.ts:4048` |
| `hud.security-section.incident-outcome` | {injured} hurt, damage {damage} of {max} | `src/content/default-locale-en.ts:4055` |
| `hud.security-section.incident-escaped` | Somebody got out. | `src/content/default-locale-en.ts:4057` |
| `hud.security-section.incidents-by-type` | By kind | `src/content/default-locale-en.ts:4059` |
| `hud.security-section.count-row` | {label}: {count} | `src/content/default-locale-en.ts:4065` |
| `hud.security-section.contraband` | Contraband | `src/content/default-locale-en.ts:4068` |
| `hud.security-section.searches-none` | No search is under way. | `src/content/default-locale-en.ts:4077` |
| `hud.security-section.search-row` | {scope} search - {state} | `src/content/default-locale-en.ts:4085` |
| `hud.security-section.search-progress` | {done} of {count} searched | `src/content/default-locale-en.ts:4091` |
| `hud.security-section.found` | Confiscated | `src/content/default-locale-en.ts:4093` |
| `hud.security-section.found-none` | Nothing has been confiscated. | `src/content/default-locale-en.ts:4103` |
| `hud.security-section.search-tally` | {found} found, {missed} missed | `src/content/default-locale-en.ts:4110` |
| `hud.severity.info` | Info | `src/content/default-locale-en.ts:4112` |
| `hud.severity.warning` | Warning | `src/content/default-locale-en.ts:4113` |
| `hud.severity.danger` | Critical | `src/content/default-locale-en.ts:4114` |
| `save.panel.region` | Prison saves | `src/content/default-locale-en.ts:4129` |
| `save.panel.title` | Prisons | `src/content/default-locale-en.ts:4130` |
| `save.manage.title` | Saved prisons | `src/content/default-locale-en.ts:4131` |
| `save.manage.local` | On this device | `src/content/default-locale-en.ts:4132` |
| `save.manage.cloud-unavailable` | Cloud saves are unavailable in this version because the game has no cloud connection. | `src/content/default-locale-en.ts:4133` |
| `save.action.create` | New prison | `src/content/default-locale-en.ts:4135` |
