...
| `hud.regime.roster-emptied` | This prison is empty. Take somebody in to start again. | `src/content/default-locale-en.ts:3497` |
| `hud.refusal.set-clock` | The clock did not change — the request was refused. | `src/content/default-locale-en.ts:3512` |
| `hud.refusal.place-build-order` | The build order was not placed — the request was refused. | `src/content/default-locale-en.ts:3513` |
| `hud.refusal.purchase-materials` | Nothing was bought — the purchase was refused and no money was spent. | `src/content/default-locale-en.ts:3514` |
| `hud.refusal.hire-staff` | Nobody was hired — the request was refused and no money was spent. | `src/content/default-locale-en.ts:3515` |
| `hud.refusal.purchase-materials-past-floor` | Nothing was bought — deliveries are refused until the prison earns the money. | `src/content/default-locale-en.ts:3538` |
| `hud.refusal.hire-staff-past-floor` | Nobody was hired — hiring is refused until the prison earns the money. | `src/content/default-locale-en.ts:3539` |
| `hud.refusal.undo` | Nothing was undone — the request was refused. | `src/content/default-locale-en.ts:3540` |
| `hud.refusal.redo` | Nothing was redone — the request was refused. | `src/content/default-locale-en.ts:3541` |
| `hud.refusal.zone-room` | The room was not designated — the request was refused. | `src/content/default-locale-en.ts:3542` |
| `hud.refusal.unzone-room` | Nothing was removed — the request was refused. | `src/content/default-locale-en.ts:3543` |
| `hud.refusal.admit-prisoner` | Nobody was admitted — the request was refused. | `src/content/default-locale-en.ts:3544` |
| `hud.refusal.admit-prisoner-no-room` | Nobody was admitted — this prison has no room to hold anybody. | `src/content/default-locale-en.ts:3560` |
| `hud.refusal.cancel-build-order` | The order is still queued — the request was refused. | `src/content/default-locale-en.ts:3561` |
| `hud.refusal.cancel-material-purchase` | Nothing was refunded — the request was refused and the delivery is still on its way. | `src/content/default-locale-en.ts:3567` |
| `hud.refusal.sell-materials` | Nothing was sold — the request was refused and nothing was taken from stock. | `src/content/default-locale-en.ts:3574` |
| `hud.refusal.release-guard` | Nobody was released — the request was refused and the guard is still assigned. | `src/content/default-locale-en.ts:3575` |
| `hud.rooms.title` | Rooms | `src/content/default-locale-en.ts:3577` |
| `hud.rooms.catalogue` | Room type and area | `src/content/default-locale-en.ts:3581` |
| `hud.rooms.catalogue-empty` | No room types are available | `src/content/default-locale-en.ts:3582` |
| `hud.rooms.selected` | Selected | `src/content/default-locale-en.ts:3583` |
| `hud.rooms.arm` | Draw on map | `src/content/default-locale-en.ts:3584` |
| `hud.rooms.disarm` | Stop drawing | `src/content/default-locale-en.ts:3585` |
| `hud.rooms.arm-hint` | Drag a rectangle across the tiles this room should cover. | `src/content/default-locale-en.ts:3586` |
| `hud.rooms.remove` | Remove rooms | `src/content/default-locale-en.ts:3587` |
| `hud.rooms.remove-active` | Stop removing | `src/content/default-locale-en.ts:3588` |
| `hud.rooms.remove-hint` | Drag across any part of a room to remove all of it. | `src/content/default-locale-en.ts:3597` |
| `hud.rooms.area` | Area | `src/content/default-locale-en.ts:3598` |
| `hud.rooms.area-none` | Nothing selected | `src/content/default-locale-en.ts:3599` |
| `hud.rooms.area-value` | {width} × {height} tiles at {x}, {y} | `src/content/default-locale-en.ts:3600` |
| `hud.rooms.confirm` | Designate {width} × {height} | `src/content/default-locale-en.ts:3601` |
| `hud.rooms.confirm-remove` | Remove {width} × {height} | `src/content/default-locale-en.ts:3604` |
| `hud.rooms.cancel` | Discard | `src/content/default-locale-en.ts:3605` |
| `hud.rooms.minimum` | Needs at least {width} × {height} tiles | `src/content/default-locale-en.ts:3606` |
| `hud.rooms.minimum-none` | No minimum size | `src/content/default-locale-en.ts:3607` |
| `hud.rooms.too-small` | Too small — this room needs at least {width} × {height} tiles. | `src/content/default-locale-en.ts:3608` |
| `hud.rooms.enclosure` | Enclosure | `src/content/default-locale-en.ts:3609` |
| `hud.rooms.enclosure-none` | Not evaluated yet | `src/content/default-locale-en.ts:3610` |
| `hud.rooms.enclosure-sealed` | Walled in — not a door check | `src/content/default-locale-en.ts:3653` |
| `hud.rooms.enclosure-open` | Open on at least one side | `src/content/default-locale-en.ts:3654` |
| `hud.rooms.requirement-enclosed` | Needs walls or doors all round | `src/content/default-locale-en.ts:3665` |
| `hud.rooms.requirement-outdoors` | Must be outdoors | `src/content/default-locale-en.ts:3666` |
| `hud.rooms.requirement-none` | No enclosure rule | `src/content/default-locale-en.ts:3667` |
| `hud.rooms.requires-object` | Needs {count} × {object} | `src/content/default-locale-en.ts:3674` |
| `hud.rooms.requires-none` | No objects needed | `src/content/default-locale-en.ts:3679` |
| `hud.rooms.coordinates` | Enter coordinates | `src/content/default-locale-en.ts:3683` |
| `hud.rooms.coordinates-hint` | The keyboard route. Dragging on the map is quicker. | `src/content/default-locale-en.ts:3684` |
| `hud.rooms.coordinates-submit` | Use these tiles | `src/content/default-locale-en.ts:3689` |
| `hud.rooms.tile-x` | Tile X | `src/content/default-locale-en.ts:3690` |
| `hud.rooms.tile-y` | Tile Y | `src/content/default-locale-en.ts:3691` |
| `hud.rooms.width` | Width | `src/content/default-locale-en.ts:3692` |
| `hud.rooms.height` | Height | `src/content/default-locale-en.ts:3693` |
| `hud.rooms.step-down` | Decrease {field} | `src/content/default-locale-en.ts:3694` |
| `hud.rooms.step-up` | Increase {field} | `src/content/default-locale-en.ts:3695` |
| `hud.rooms.needs` | Not ready | `src/content/default-locale-en.ts:3700` |
| `hud.rooms.needs-count` | {unfinished} of {total} | `src/content/default-locale-en.ts:3701` |
| `hud.rooms.needs-room` | {room} at {x}, {y} is missing | `src/content/default-locale-en.ts:3706` |
| `hud.rooms.needs-object` | {count} × {object} | `src/content/default-locale-en.ts:3710` |
| `hud.rooms.needs-object-uncounted` | {object} | `src/content/default-locale-en.ts:3715` |
| `hud.rooms.needs-item-more` | and {count} more | `src/content/default-locale-en.ts:3720` |
| `hud.rooms.needs-object-unknown` | something this build cannot name | `src/content/default-locale-en.ts:3724` |
| `hud.rooms.needs-doorway` | a door — nobody can get in | `src/content/default-locale-en.ts:3751` |
| `hud.rooms.needs-unreachable` | a way in — nothing outside can reach its door | `src/content/default-locale-en.ts:3793` |
| `hud.rooms.at-capacity` | At capacity | `src/content/default-locale-en.ts:3853` |
| `hud.rooms.at-capacity-count` | {full} of {total} | `src/content/default-locale-en.ts:3854` |
| `hud.rooms.at-capacity-room` | {room} at {x}, {y} is full | `src/content/default-locale-en.ts:3855` |
| `hud.rooms.at-capacity-places` | places in use: {inUse} of {capacity} | `src/content/default-locale-en.ts:3856` |
| `hud.security-section.title` | Security | `src/content/default-locale-en.ts:3878` |
| `hud.security-section.waiting` | No prison is reporting. | `src/content/default-locale-en.ts:3887` |
| `hud.security-section.sectors` | Sectors | `src/content/default-locale-en.ts:3890` |
| `hud.security-section.sectors-empty` | No sector has been drawn on this land yet. | `src/content/default-locale-en.ts:3897` |
| `hud.security-section.sector-staffing` | {assigned} of {required} guards assigned | `src/content/default-locale-en.ts:3904` |
| `hud.security-section.sector-short` | {count} short | `src/content/default-locale-en.ts:3911` |
