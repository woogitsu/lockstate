...
32. **Build costs are material quantities, and that part is real**:
    ...
    **The one thing about an order that does now cross is a refusal** (#261,
    ...
    never both and never neither. Gap 34 records why the difference matters at
    720px and below.
...
34. **A refusal cannot be dismissed, and carries no location.** The alerts
    ...
    *why* but not *where*; carrying a position would put a second copy of the
    ...
    A purchase is the one command with a surface on each side of that line,
    and it is worth stating which player sees what. `src/main.ts` refuses an
    unaffordable or unpriced purchase *before* dispatching, in the band, on
    the pressed control; `ProcurementSystem` refuses what got past that, into
    this list. So at 720px and below a player still sees the affordability
    refusal they can provoke by asking for more than the prison has ever had,
    and sees nothing at all about the two cases only the worker catches — a
    run of presses inside one tick, and a balance that moved since the last
    publication (gap 32).
