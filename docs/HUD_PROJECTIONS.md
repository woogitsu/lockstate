...
32. **Build costs are material quantities, and that part is real**:
    ...
    **The one thing about an order that does now cross is a refusal** (#261,
    ...
    > dispatches nothing", and that half is no longer true.** Since ADR XXXX
    > (*"What a player sees for an order given while the clock is paused"*,
    > drafted with a placeholder number and to be renumbered on landing) the
