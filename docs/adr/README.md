...
**Next free number: 0064.** 0055 and 0058 were held by parallel drafts when this sentence was written — *0058's draft was never written and the number is a gap; see the correction later in this paragraph, which is the one to believe* — and 0056 and 0057 were assigned to the command-ordering ADR and the riot-regime ADR in the same 2026-08-28 pass, so this line is a ceiling over a gap rather than the next gap — read the table. 0049 through 0054 were all assigned in the 2026-08-28 integration pass, before their drafts existed, which is the change that stopped them colliding: 0049 to the payroll-arrears ADR, 0050 to the sentence-end ADR, 0051 to the paused-clock ADR, 0052 to the source-art ADR and 0053 to the post-eligibility ADR. **This line read "Next free number: 0058" until 2026-08-28**, when 0062 was assigned to the contention-fairness ADR and became the highest number on disk; the contract is `max + 1`, so the line moved to 0063 rather than to the lowest gap. 0058 and 0060 were handed out that day and returned unused — the keyboard work and the needs work each correctly declined to write an ADR rather than manufacture one — and 0059 and 0061 are held by drafts in flight, so 0055 and 0058 through 0061 are gaps rather than the next number. Read the table. **It then read "0063" for the rest of that day**, until 0063 was assigned to the restore-refusal ADR and took the maximum with it; both readings are marked rather than overwritten because the reason each was right is the same rule, applied to a different maximum. The two drafts in flight each computed `0063` too, correctly and before this file existed — three branches agreeing on one next-free value is what `max + 1` produces when every number being added is below the current maximum, and it is not a collision. Whichever of the three lands last moves this line. The gaps this paragraph used to record are gone:
