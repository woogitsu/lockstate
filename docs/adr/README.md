...
already implements it — each of these arrived on the same branch as its
...
a `Proposed` row is **a signal rather than one of a crowd**, so a short list
reads as requests addressed to the owner rather than as background. The count of
ADRs awaiting approval is two, and it is tracked rather than untracked, which
was the point.
...
| [0030](./0030-restoring-an-interrupted-incident-response.md) | What a restored session owes an incident response that was interrupted by a save | Proposed — pending human approval |

**Next free number: 0031.**

**Two rows are `Proposed`: 0029 and 0030.** Both arrived with the change that
implements them, which is the shape [`STATUS-QUEUE.md`](./STATUS-QUEUE.md) §2
names as the fragile case; both queue entries are in the same commit as their
document, per that file's rule. The sentences below about the queue being empty
were true when they were written and §2 of that file records what this costs.
0030 is the one of the two whose approval is *load-bearing before merge* rather
than after: two of its three decisions rewrite a player's stored save the first
time it is loaded and cannot then be unmade.
...
checking, and **0030's author found the opposite**: two pull requests were open
(#355, and #356 which changes 0029's status and adds no document), so the check
was the one that mattered rather than a formality that happened to pass. Neither
was holding 0030.
