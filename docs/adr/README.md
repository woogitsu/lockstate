...
## Why 0025 is absent, and 0028 is not the next number after 0027

One number between 0024 and 0028 is skipped here and it is not free. `0025` is
claimed by PR #302, open at the time of writing and therefore invisible to this
table, which is a statement about `main` and can never be anything else. `0024`,
`0026` and `0027` were in that list until they merged, which is what closing the
gap from below looks like — 0026 and 0027 arrived together, from PR #299, as the
rows above.

That invisibility is the whole reason a gap exists at all. This file says of
itself that "the number is not reserved until it appears below", which is true
and is also the trap: three agents collided over 0024 and 0025 before 0028 was
taken, because each read the stated next-free number on `main` and each was
right. So 0028 was allocated out of band — by the owner, across the open
...
0026 and 0027 are what the other half of that collision cost. PR #299 wrote
them as 0024 and 0025, on the same reading of the same stated next-free number,
and had to renumber once 0024 merged and 0025 was claimed — two files, two
headings and seven citations across `docs/`, `src/` comments and `tests/`, none
of which any assertion would have caught as *wrong* had they been left pointing
at somebody else's ADR.
...
is a ceiling, not a reservation**: whoever writes the ADR after 0028 should check
the open pull requests before taking 0029, exactly as 0028's author had to.
