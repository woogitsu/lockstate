...
**Two ADRs in this directory are `Proposed`: 0029 and 0030.** A `Proposed` ADR
is one awaiting the owner's approval, and is not binding whether or not code
already implements it — each of these two arrived on the same branch as its
implementing change, and that does not make either accepted.

This paragraph used to read *"No ADR in this directory is `Proposed`, as of
2026-08-25"*, and then *"Exactly one … 0029"*. The property it is recording is
still the one that matters: a `Proposed` row is **a signal rather than one of a
crowd**, so two rows read as two requests addressed to the owner rather than as
background. The count of ADRs awaiting approval is two, and it is tracked rather
than untracked, which was the point.
...
| [0030](./0030-build-queue-cancellation-surface.md) | Withdrawing one queued build order — where a player aims, and what a long queue looks like | Proposed — pending human approval |

**Next free number: 0031.**

**Two rows are `Proposed`, and they are the only two.** Both arrived with the
change that implements them, which is the shape
[`STATUS-QUEUE.md`](./STATUS-QUEUE.md) §2 names as the fragile case; each has a
queue entry in the same commit, per that file's rule. The sentences below about
the queue being empty were true when they were written and §2 of that file
records what this costs.

**0030's number was taken without the open-pull-request check 0029's author
made.** `gh` is not installed on the machine that wrote it, so
`gh pr list --state open` could not be run, and the number is this table's stated
next-free and nothing stronger. That is the condition under which 0024 through
0027 collided, so it is written down rather than glossed: if an unmerged branch
is holding 0030, 0030 is the document that renumbers.
