...
| [0049](./0049-what-a-prison-that-cannot-make-payroll-owes.md) | What a prison that cannot make payroll owes | Accepted for decisions 2-6, 2026-08-31 — **this cell read `Proposed, 2026-08-28` until the owner accepted it on 2026-08-31 in [#703](https://github.com/matmaxalez/lockstate/issues/703) ruling 15**, having been `Proposed` while five of its six decisions ran in production. **[ADR 0075](./0075-what-a-prison-that-cannot-afford-its-first-bed-is-owed.md) decision 2 REPLACES decision 1** (0075 is Accepted, 2026-08-29), and it gives `Treasury` the signed balance, overdraft and loan that decision 1 refused it — and until 2026-08-31 **no note of the contradiction existed anywhere**: `grep 0075` over the document returned zero. This document named its own reversal in advance, in its *what would change my mind*: *"A loan is an overdraft in different clothes … otherwise the argument above collapses."* ADR 0075 decision 2 did both halves, so the condition was met by the letter. `adr-status-reference-contract` cannot catch this class — it compares a *cited* status to its document, and here no sentence claimed a false status; the decision was contradicted with no citation at all. **Decisions 2-6 stand and are what this is accepted for**: arrears remain a second non-negative integer beside the balance, accruing rather than overdrawing, and `PayrollSystem` still pays `min(due, balance)` — ADR 0075's floor is one the owner may lower, not a licence for payroll to overdraw. Proposed, 2026-08-28 — answers ADR 0042 open question 3, which ADR 0017 answer 3's *"None of that is licensed to be decided in implementation code"* forbids answering in a docblock. The treasury stays floored at zero and the unpaid part of a wage bill becomes **arrears**, a second non-negative integer beside the balance: a treasury that could overdraw would pay the wages out of debt and ADR 0017's ladder would never reach its bottom rung. Measured over 30 in-game days, the debit costs one part in thirty of income at the staffing the game asks for, and break-even needs thirty times it|
