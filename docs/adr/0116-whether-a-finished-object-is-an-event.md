...
`src/simulation/protocol/types.ts:2157-2186`; the correction under the table
...
> `src/simulation/protocol/types.ts:2109` and closes `] as const;` at `:2002`,
...
`roomNameKey: identifierSchema` (`src/simulation/protocol/types.ts:2482`) is
...
- **`:1864-1892` was right at `ca82e946` and is wrong at `460ef077`.** The
  ...
  reads `src/simulation/protocol/types.ts:2157-2186` with
...
- **A completion is not a route decision, and could not become one cheaply.**
  ...
  (`src/simulation/refusals/refusal-log.ts:318-391`), which compares the route
