...
**RULED BY THE OWNER ON 2026-09-16: THIS DOCUMENT IS LIVE, NOT HISTORY — ITS
ANCHORS ARE MAINTAINED LIKE ANY OTHER ADR'S.** The Status above does not move:
it is still `Proposed`, still not accepted and still not self-approved. What was
ruled is a different question, which nobody had ever answered and three
in-flight branches were answering by assumption.

**The question, in one sentence.** Decision steps of this document have landed
through *other* ADRs rather than through it — `docs/adr/README.md`'s own rows say
so, [ADR 0048](./0048-what-a-sectors-occupants-are.md) *"takes ADR 0042 decision
2 and answers its open questions 1 and 2"* and settles `needsPressure`, which is
step 4's subject, and [ADR 0049](./0049-what-a-prison-that-cannot-make-payroll-owes.md)
*"answers ADR 0042 open question 3"*, which is step 3's — so the tree this
document diagnoses has been replaced under it while its own decision was never
taken. (**#1240 puts it at four of the six and that count is its, not re-taken
here**; the ruling does not turn on the number.) Is it therefore **history** — frozen, its
`file:line` citations read as of the day they were written, on the precedent
[#1229](https://github.com/woogitsu/lockstate/issues/1229) set for ADR 0040 — or
is it **live**, and its anchors a maintenance obligation like any other
document's? The owner chose:

> Żywy — przepinajcie dalej (zalecane)

("Live — keep re-pinning.")

**The provenance is the weaker of the two kinds this repository distinguishes.**
The owner did not type a sentence; they chose a clickable option whose label an
agent session had written, and **the label is the whole of what was agreed** —
the same provenance `CLAUDE.md` flags for the 2026-09-08, -09 and -10 releases
inside reservation 3, and the same
[ADR 0091](./0091-what-clears-the-refusal-band.md),
[ADR 0116](./0116-whether-a-finished-object-is-an-event.md) and
[ADR 0115](./0115-where-the-prisoner-roster-lives-and-what-the-manage-rail-can-afford.md)
record of themselves. It agrees with a recommendation rather than overriding
one, which makes it the weaker sort of record again and is said here rather than
left out.

**What it means for an agent sweeping this file, which is why it is recorded at
the top rather than in a research note.** The ADR 0040 precedent — leave a dead
diagnosis' anchors where they are, so a superseded document does not read as a
description of today's tree — **does not apply here**. An anchor below that
still names live code is to be re-aimed; a sentence that is simply false about
the tree is to be marked in both directions (`docs/AGENT_WORKFLOW.md` §4) rather
than left standing under a pin that says it was read. Freezing this document was
the declined option.

**The ruling was asked for by the work it governs, and that is recorded rather
than implied.** [#1240](https://github.com/woogitsu/lockstate/pull/1240) — the
pinned-ADR packet that sweeps this file — names as its own weakest claim *"that
leaving 55 anchors in ADR 0042 … is case 4 rather than work declined"*, observes
that this ADR *"is `Proposed`, its own decision was never taken, and the tree
moved under it by other ADRs' hands"*, and says in terms what would settle it:
*"the owner ruling on ADR 0042"*. This is that ruling, and it settles it against
the branch's own choice. **That branch is in flight and nothing here has been
pushed to it**; the 55 anchors are now a re-pinning job somebody owes, not a
recorded decision to leave them. [#1238](https://github.com/woogitsu/lockstate/pull/1238)
and [#1239](https://github.com/woogitsu/lockstate/pull/1239) are sibling packets
from the same pocket and touch other ADRs — **they do not edit this file** — but
they apply the same freeze-or-repin policy to theirs, and the ruling is about
this document only.
...
(`src/simulation/prisoners/intake-system.ts:607`; the anchor read `:307`). Every
reader in `src/`:
...
call site, at session creation (`src/simulation/runtime/new-session.ts:439`; the anchor read
`:281`), so
...
(`src/simulation/runtime/new-session.ts:1448`; the anchor read `:622`), and the
only writer of that
...
read `src/simulation/runtime/new-session.ts:611-624` — the function moved file,
...
(`src/simulation/runtime/new-session.ts:636`). `action.sleep` restores
...
- `IncidentResponseSystem` — the whole `active → notified → responding →
  ...
  at `src/simulation/runtime/new-session.ts:1637` (the anchor read `:662`). Its `update` iterates
...
- `ClassificationReviewSystem` — registered via
  ...
  (`src/simulation/prisoners/prisoner-operations-runtime.ts:545`; the anchor read
  `:142-147`), folds
  ...
  (`src/simulation/prisoners/classification-review-system.ts:445-449`; the anchor
  read `:229-233`).
...
>   (`src/simulation/runtime/new-session.ts:1625`), and the save carries
...
`src/simulation/runtime/new-session.ts:623`, the file the function left). No RNG.
...
- **If a step needs a draw, it takes a named stream.** `NamedRngStreams.get`
  ...
  (`src/simulation/runtime/new-session.ts:447`, `:483-487`; the anchor read
  `:288-293`) — today
...
- **No step may reorder an existing iteration.** `resolveSectorOccupants` sorts
  ...
  `src/simulation/runtime/new-session.ts:623`),
