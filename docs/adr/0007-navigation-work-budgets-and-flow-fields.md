...
## Amendment, 2026-09-23: the budget is charged what a request costs cold, not what the caches saved it (issue #1373)

**The decision above, "sums each *fully computed* request's expansions
against a configured `workBudget`", is changed in what it sums, and in
nothing else.** The currency is still expanded search nodes. The granularity is
still the request. Forward progress still wins. What changes is which nodes a
request is charged for.

### Why

The owner ruled on 2026-09-23, on [ADR 0059](./0059-how-an-actor-gets-from-one-tile-to-the-next.md)
open question 3, that a save carries the work in flight, so that save, restore
and continue is the same game as continue. The option was labelled *"Zapisuj
marsz (zalecane)"*; that is an option label, the weaker provenance. The queue
and its uncollected routes are carried. The caches are not: this ADR's
"Consequences" and `docs/PERSISTENCE.md` keep them derived.

**The caches were not budget-neutral.** A `RouteCache` or `FlowFieldCache`
hit cost 0 expansions. A restore starts both caches empty. So a restored
session paid for searches that the saved session had already paid for before
the save.

**Measured on the same tick's pending queue, replayed warm and cold.** The
expansions differed on 76 of 331 ticks where the budget bound. The spread was
populations of 18 to 36 prisoners in six rows of cells, with the budget binding
at regime block changes. The largest difference was 137 expansions. The set
served differed on 13 of those ticks.

**Measured through a real save and restore.**
- 24 prisoners, admitted 7 ticks apart: every save taken 1 to 40 ticks before
  the tick-3461 block change served a different set, visible at tick 3463.
- 36 prisoners, admitted 7 ticks apart: every save taken 1 to 600 ticks before
  tick 5801 did the same.

`tests/determinism/restore-mid-walk-exactness.test.ts` pins both cases. Before
this amendment they failed 5 and 7 saves.

### What is charged now (`PathRequestQueue.processTick`)

- **The region pass, once per flow-field group per tick**, at the group's
  first request served. The charge is `RegionFlowField.computationExpansions`,
  whether the field was computed this tick or came from the cache. The field is
  a pure function of the graph, the doors, the destination and the context, so
  the two numbers are the same.
- **Each leg, by the method this tick's company selects.** A group at or over
  `flowFieldActivationThreshold` routes through the field; otherwise the leg
  uses `findRoute`. On a miss the leg is charged what it cost. On a hit it is
  charged the price the entry recorded for that method:
  - that price is valid while the entry is, because the entry's door
    dependencies are every door the search consumed, with both verdict and
    cost;
  - for the field method the price is also tied to the field object it was
    measured against, because a leg's dependencies are narrower than its
    field's;
  - an entry with no price for the current method is measured once by
    recomputing the leg, and the result is discarded. The cache-agreement and
    shared-plan tests pin that it is the same answer.

**Every term is a pure function of the pending set, the world and the doors,
so a warm cache and a cold one charge the same.** The caches still save the
CPU; `expansions` and `totalExpansions` still report the work actually done;
and the answers are unchanged.

### What it costs

- **Budget binds at least as often.** A warm session can now defer a request
  it used to serve on the same tick, because a hit is no longer free against
  the budget. Two pinned figures moved:
  `tests/integration/incident-trigger-reachability.test.ts`'s 96-prisoner
  sector-risk scores went from 0.7746 to 0.762 and from 0.8036 to 0.814, with
  its riot counts unchanged. Nothing else in the suite moved.
- **Extra CPU on some hits.** When every leg of a group hits the route cache
  but the field is no longer cached, the field is computed just to price the
  region pass. A hit whose price for the current method is unknown also
  recomputes its leg once.
- `flowFieldActivations` still counts only the groups a leg was actually
  computed through (`tests/unit/navigation-work-budget-and-cache.test.ts`).

### Alternatives considered

- **Persist the caches.** Rejected. They are unbounded. `RouteCache` keeps an
  entry per distinct leg until the geometry or a consumed door changes, which
  at a 3,000-prisoner scale is on the order of beds times destinations, each
  entry a whole route. They would put a history of queries into every save and
  into its checksum, and this ADR and `docs/PERSISTENCE.md` both keep them
  derived for that reason.
- **Change the currency to something that needs no search.** Examples are
  route length or a fixed cost per request. Rejected: it would no longer bound
  the thing the budget exists to bound, and it would retime every tick on which
  the budget binds.
