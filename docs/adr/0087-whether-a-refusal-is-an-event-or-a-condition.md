...
| --- | --- | --- | --- | --- |
| A1 | `src/main.ts:2753` — `purchase-materials` pre-flight, via `judgeAffordability` (`src/ui/affordability.ts:167-189`) | the charge, the last published balance (≤500 ms old), the `'deliveries'` rung floor (`affordability.ts:152`) | `hud.refusal.purchase-materials-past-floor`, or the generic `hud.refusal.purchase-materials` | **your press failed** |
| A2 | `src/main.ts:2975` — `hire-staff` pre-flight, same function | as A1, at the `'hiring'` rung, which shares the `'deliveries'` threshold | `hud.refusal.hire-staff-past-floor` / `hud.refusal.hire-staff` | **your press failed** |
