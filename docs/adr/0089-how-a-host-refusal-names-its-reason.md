...
> reason (`src/ui/hud/projection.ts:1372`,
...
> (`src/content/default-locale-en.ts:2396`, key `hud.refusal.admit-prisoner-no-room`,
...
(`src/content/default-locale-en.ts:2380`, key `hud.refusal.admit-prisoner`).
...
| H4 | `src/main.ts:2998-3001`, `admit-prisoner` — no room instance exists | genuinely player-actionable: nothing is zoned yet | **At the time, no — plain `Error`, the #791 defect. Fixed 2026-09-03, Option-1-shaped — see the status note.** | `hud.refusal.admit-prisoner-no-room` (`src/content/default-locale-en.ts:2396`): "Nobody was admitted — this prison has no room to hold anybody." — shipped, not `hud.alert.refusal.admit.no-accommodation`'s wording as guessed below |
