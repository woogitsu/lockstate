...
5. **faulted**: Reached when an unhandled exception occurs. The worker emits a `protocol/error` and refuses to process further ticks or commands. The renderer must detect this and surface a restart/reload UI. A protocol decode error does *not* reach this state: [ADR 0024](./0024-protocol-fault-recoverability.md) settles that a message the decoder rejected reached no simulation state, so it is faulted with `recoverable: true` and the worker keeps its session.
...
- **No client sends a `protocol/handshake`.** Every occurrence of that message kind in `src/` is the receiver, the kind union or the schema; the only senders in the repository are under `tests/`. The main thread's first message to a worker is `simulation/initialize` (`src/persistence/session/worker-session-host.ts:142`), which `handleInitialize` (`src/simulation/worker/state-machine.ts:665`) accepts because the state is still `'uninitialized'` (`:666-668`). [ADR 0003](./0003-simulation-worker-protocol.md)'s decision 4 carries the same note, and its anchors were re-measured in the same sweep as these.
- **`'ready'` is unreachable.** `SimulationWorkerStateMachine` declares six states (`state-machine.ts:33-39`) and writes `_state` in exactly one place, `transition()` (`:270`, inside the method declared at `:269`). Its four call sites reach `'faulted'` (`:590`), `'paused'` (`:725`), `'paused'`/`'running'` (`:760`) and `'shutting-down'` (`:984`). `handleHandshake` (`:637`) posts `protocol/handshake-accepted` and transitions nothing, so even a handshake that *was* sent would not advance the state.

**Every anchor in the two bullets above was re-measured; all nine had drifted, by up to 332 lines, and the finding is unchanged.** The substance was re-established and not assumed: `this.transition(` still appears exactly four times in `state-machine.ts`, still at those four targets, and still never with `'ready'`; `_state` still has exactly one assignment outside its initializer. The anchors used to read `state-machine.ts:25-31` for the states, `:191-192` for `transition()`, `:400`/`:508`/`:543`/`:652` for the call sites, `:444-460` for `handleHandshake`, and `:137-144`/`:472-474` for the initialize path. **Where the old anchors now land is the reason to say so rather than fix them silently.** `state-machine.ts:400` is now inside a comment about refusal-driven projection publishing, and the `:444`-`:460` range this bullet gave for `handleHandshake` now covers the body of an unsolicited `simulation/status-counts` post whose own comments cite ADR 0003. Both read as though they were about this ADR's subject while containing none of it. An anchor that drifts onto plausible-looking code is worse than one that drifts onto nothing, because it invites the next reader to report the *finding* as stale rather than the citation.

The Positive consequence below — "main thread cannot send commands before the kernel is ready" — still holds, by a different route than this list implies: `simulation/submit-command`, `simulation/request-snapshot` and `simulation/request-projection` fault with `not-initialized` when no kernel exists, and `simulation/set-clock` faults with `invalid-state` outside `paused`/`running`. What does not hold is that a handshake is the gate.

**Implementation note, 2026-08-30: those four guards no longer reach state 5, and the sentence above never said they did — it is marked rather than rewritten because a reader could reasonably have assumed it.** Until [issue #680](https://github.com/matmaxalez/lockstate/issues/680) all four called `fault()` without options, so they took its `recoverable: false` default and moved the worker to `faulted`. The page's panel readers are built at boot over the channel rather than over a session (#149), so the first `.ui-tab` press asked the **boot worker** — the one `SimulationWorkerChannel.claimForSession` hands to the first session — for a projection; the guard fired; and the player's first *New prison* was then refused `already-initialized`, *"Kernel is already initialized."*, about a worker that had no kernel. They now report `recoverable: true`, which is what state 5 above already described — *"Reached when an unhandled exception occurs"* — and what a guard that fires before any kernel exists has always been: a refused request that touched no simulation state, the condition [ADR 0024](./0024-protocol-fault-recoverability.md) §1 settled for the decode path. The refusals themselves are unchanged: same codes, same `replyTo`, same messages.
