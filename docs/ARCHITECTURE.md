...
The `Kernel` orchestrates this logic using deterministic system ordering, driven at a fixed step by `FixedStepClock` — the 50 ms is the clock's, not the kernel's: `src/simulation/kernel/kernel.ts` holds no time value at all and `step()` takes no argument, while `src/simulation/clock/fixed-step-clock.ts:29` carries the default and `src/simulation/worker/state-machine.ts:165` supplies it. See [DETERMINISM.md](./DETERMINISM.md) for full details on tick semantics, command ordering, and RNG ownership.
