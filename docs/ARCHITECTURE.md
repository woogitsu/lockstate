...
Two of them reach the status strip today, both over a worker-to-main publication rather than a snapshot poll. The worker publishes the tick (`simulation/clock-state`) and the composition root runs `projectClockPosition` on it; the worker runs `projectStatusStrip` itself and publishes its counts (`simulation/status-counts`). Every *other* projection has a route as well, and it is one route rather than a third and fourth special case: `simulation/request-projection` names a read model from a closed vocabulary and `simulation/projection` answers it, correlated, with the tick it read and the page window it built. That channel is a pull — nothing publishes a projection on a timer — and it has three readers: `src/ui/simulation-room-needs.ts` asks `hud/room-list` and `hud/room-detail` while the Rooms tab is showing, so the panel can say what a zoned room is missing; `src/ui/simulation-build-queue.ts` asks `hud/build-queue` while the Build tab is showing, so the panel can say what is still coming and let one order of it be withdrawn; and `src/ui/simulation-intake.ts` asks `hud/prisoner-population` while the Overview tab is showing, so the Intake panel can say where the arrivals it admitted are — waiting for a cell, or in the terminal stage nothing releases. The second is the one that made a *command* reachable rather than only a readout: `CancelBuildOrder` names an order id, no order id reached this thread, and it was the repository's last command with no production producer. Nine of the thirteen read models still have a route and no reader. HUD_PROJECTIONS.md sections 8 and 9 give both halves, and `tests/foundation/projection-reachability-contract.test.ts` is what fails if a projection is added with no route at all.
