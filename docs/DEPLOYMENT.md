...
Lockstate is delivered as a Cloudflare Worker with Static Assets, through the official Cloudflare Vite plugin. The browser simulation remains client-side. No Node.js or PHP application server is required in production, and a server-side Worker entry point must not be added merely to serve the SPA.

**This paragraph said "assets-only" until 2026-09-03**, and it stopped being true in the commit that added `main` to `wrangler.jsonc` — the telemetry ingest entry point the owner authorised that day (see "The first server entry point lands with the ingest, not before" below, and `AGENTS.md` for their words). The last sentence is unchanged and still binding: the entry point exists to carry ADR 0046's ingest, and it is not a licence to put a handler in front of the SPA for any other reason. There is exactly one route and it answers only when a deployment names its path.
...
> **That paragraph was re-derived on 2026-09-15 and every number in it holds** — 38/14/8 to the run, matched independently off the Actions API — **and it is not the rate.** It is quoted rather than overwritten, because it is true about the window it names and the window was a bad day. Over the whole of `main`'s history the figure is **65 cancelled of 818 push-started CI runs, 7.9 %** against that window's 13.3 %, and it arrives in bursts: eleven of the twenty-five days have none at all, and 2026-09-08 lost twelve of nineteen. **Forty-seven of the 65 recorded zero jobs** — the pending-slot eviction `ci.yml`'s concurrency comment describes — and the other eighteen were cancelled after starting, which is a different event with an unestablished cause. The full census, the per-day table and what it cost is [`docs/research/2026-09-15-what-main-never-published.md`](./research/2026-09-15-what-main-never-published.md). **This file had not changed since 2026-09-04 and several ADRs cite it; the correction is additive and the sentence above is the one they cite.**

> **Two further things that record establishes, and they belong here rather than only there.**
>
> **`staging-blocked` demonstrably fires.** Every one of the forty-nine cancelled `main` CI runs since #424 landed produced a Deploy run that concluded `failure` — thirty-seven of them, with the remaining twelve predating #424 and concluding `skipped`. Four were confirmed by job *name* rather than by conclusion: Deploy `34996522855`, `34996533665` and `34999457654` each report one failed job named `staging-blocked`, while `33005039649` — the child of `33005035205`, one of the two runs `ci.yml`'s comment cites — has only `staging` and `production`, both `skipped`. That is the before-and-after of #424, measured.
>
> **A Deploy run's `head_sha` is not the commit it published.** A `workflow_run` workflow runs against the default branch's tip, so the Deploy run carries `main`'s tip at trigger time while its `staging` job checks out `workflow_run.head_sha`. Anyone auditing what reached the Worker by reading Deploy `head_sha` values will get the wrong commit, and the mistake is invisible because the two agree whenever nothing merged in between. Match by the triggering run instead.

**How long to wait between merges, so that no `main` run is ever evicted.** The eviction is a three-merge race: one run in progress, one pending, and the third merge cancels the pending one — so two merges may be in flight and the third is the one that kills. With merges spaced `S` apart and a `main` run taking `D`, the third arrives at `2S` and must find the first finished, which is **`S ≥ D / 2`**. Measured 2026-09-15: `D` is a **median 21.2 min** and a p90 of 66.9 min over the thirty-six attempt-1 `main` runs since 2026-09-13, and **no zero-job cancellation in all 818 runs ever had its successor more than 16.42 minutes behind it**. So the rule is **twenty minutes between merges to `main`, thirty-five when the suite is already running long** — or, in the form that needs no clock and is exact, **do not merge while two `main` CI runs are unfinished**. This is a rule for whoever is merging; nothing in `.github/workflows/` enforces it, and the workflow-side repair is stated and declined in the research record's §6 because that directory is the owner's.
...
**Decided by the owner on 2026-08-27, and recorded here because this is the section a person checks before touching what serves the live site.** This paragraph read *"Nothing in this repository executes it yet: there is still no `main` in `wrangler.jsonc`, no Worker handler anywhere, and this section adds none"* until 2026-09-03. **It landed on 2026-09-03**: `wrangler.jsonc` declares `main`, `src/worker/` holds the handler, and the nine items below are worked out under "What actually landed". The 2026-08-27 decision about the *order* is left standing above because it is what put the change here rather than after a staging split.
...
#### What actually landed, 2026-09-03 — the nine items, answered

**The owner authorised the merge on 2026-09-03.** The decision was put to them as three options — prepare the change for review without merging it, leave the surface blocked, or build it and merge it — and they answered *"Zbuduj i zmerguj"* ("build it and merge it"). `AGENTS.md`'s "The owner's standing mandate" records that in their words, dated, beside the reservation it releases. **The release covers this ingest and nothing else about server-side execution.**

The checklist above asks for nine sentences. Here they are, against the change as built. Items 3, 4 and part of 5 are answered by **absence**, and that is the most important thing in this subsection: the endpoint accepts nothing yet.

1. **What lands on `lockstate.io`.** The commit that adds `main` to `wrangler.jsonc` and `src/worker/`. Merging it publishes a Worker with an executing `main` to the live site on the next CI success, with no further approval.
2. **Which requests the handler claims.** Exactly one path, and only when the Worker binding `LOCKSTATE_TELEMETRY_INGEST_PATH` names one — which no environment in `wrangler.jsonc` and no workflow in `.github/workflows/` does. **With it unset there is no route at all and every request falls through to Static Assets unchanged.** `assets.not_found_handling` is still `single-page-application` in all three environments. Note the one thing the checklist did not anticipate: `assets.run_worker_first` has to be `true`, because SPA handling makes the asset router match every path and a Worker would otherwise never be reached — so the handler is in front of every request including each `/assets/*` file, and its last statement is `env.ASSETS.fetch(request)`.
3. **What the Worker holds.** One string: the ingest path. **Not the destination and not a database credential**, because neither exists yet.
4. **What it must not hold.** It holds no credential of any kind, so the service-role prohibition is met by there being nothing to hold. When a credential arrives it is a dedicated least-privilege database role and `supabase/tests/003_data_api_grants.test.sql` gains it in the same change — that suite names its roles as literals and will not otherwise see one. No `VITE_` name is involved: the binding is `LOCKSTATE_`-prefixed and is a path, not a secret.
5. **What it must do before it stores anything.** Server-side validation against the versioned envelope schema — through the *same* `admitTelemetryEnvelope` the browser sink calls, so there is one definition of an admissible envelope rather than two — plus a bounded body (declared *and* delivered), a bounded batch, strict UTF-8, a `.strict()` wire schema, and refusal of text no destination can store. The stored occurrence time is the server's (`receivedAt`, from an injected clock) and the weight is the receiver's own registry row; the caller's values travel as `claimedOccurredAt` and `claimedSampleRate` and are keyed on and multiplied by nothing. **Request rate is the one bound not implemented here**, and deliberately: a limiter has to key on something, and the only value available is the caller's IP, which ADR 0046 §7 item 2 forbids retaining. Rate limiting belongs at the edge, in configuration, which is the owner's.
6. **Whether `public/_headers` changes.** **It does not, and it needs no change.** The ingest is same-origin, so `connect-src 'self'` is intact. What `_headers` does not reach is a response the Worker writes itself — it is applied by Static Assets — so the handler sets its own `content-type`, `cache-control: no-store`, `x-content-type-options: nosniff` and `referrer-policy: no-referrer`, and deliberately sends no `Access-Control-Allow-Origin`. Asset responses still come from Static Assets and still carry every header `_headers` declares; `scripts/verify-deployment-preview.mjs` asserts that through workerd with the Worker in front.
7. **Rollback.** Reverting the commit that adds `main` restores an assets-only Worker, and the deploy that follows publishes it; anyone who can merge to `main` can run that, and it needs no Cloudflare access. Three things stand between a defect and a dead site in the meantime: route resolution cannot throw and answers `undefined` for any binding that is not a legal path, the ingest call is wrapped so a throw is one status code on one path, and the fall-through is the last statement in the handler.
8. **The trap.** Untouched and still true: do not dispatch the `production` job. It transfers `lockstate.io` onto the Worker `lockstate`, which has never been deployed, and wrangler does not warn.
9. **ADR 0002's amendment.** Written in the same commit, as a dated `Amendment` section that quotes the Decision bullet rather than overwriting it. See below.

**What is still owed before telemetry actually collects anything.** Three things, none of them in this repository's gift: the `supabase/migrations/` content (a `telemetry_events` table, a `SECURITY DEFINER` insert function, the dedicated role, and the retention job — which ADR 0008's scope amendment puts *inside* §3, unlike the ingest); the two Worker variables that name a destination and hold its credential; and the client-side pair `LOCKSTATE_TELEMETRY_INGEST_PATH` / `LOCKSTATE_TELEMETRY_ENVIRONMENT`. **The client path and the Worker path must be set to the same value.** Nothing can enforce that agreement — both are validated against one regex, so they cannot disagree about what is *legal*, but a mismatch means the client posts to a path the Worker does not claim, Static Assets answers the SPA shell with `200`, and the sink counts every dropped batch as delivered. Set the Worker binding first.

**Shrunk on 2026-09-04, and the paragraph above is left standing because its first item is the one that changed and a reader needs to see what it used to demand.** Its parenthesis listed four things owed under `supabase/migrations/`. All four are now written, in one file, `supabase/migrations/20260904090000_create_telemetry_events.sql` — the table, the insert function, the `telemetry_ingest` role, and the retention job, which is a function plus the audit table ADR 0008 §3 step 6 makes it owe. The owner authorised the fourth on 2026-09-04 in the words that file quotes and dates; the first three were authorised the day before. `supabase/tests/003_data_api_grants.test.sql` gained the role in the same change, which is what the checklist's item 4 asked for and what this section predicted that suite would not otherwise see.

**What that leaves, and it is smaller rather than gone.** Three things, in the order they have to happen:

1. **Applying the migration**, which is ADR 0016's mechanism and the owner's. Nothing in it has been applied anywhere. `pnpm verify:sql` executes the whole directory against a local PostgreSQL 16.13 with pgTAP and passes 412 assertions; that is the only place any of this has run.
2. **A credential and a destination** — the two Worker variables, unchanged from the paragraph above, plus the decision of *how* the Worker becomes `telemetry_ingest`: that role is `nologin` with no password, deliberately, because a password in a migration is a credential in the repository. Either a password set outside this repository or `grant telemetry_ingest to authenticator` for a PostgREST call, and both are deploy configuration.
3. **A schedule for the retention job, which is one command and is not in the repository.** The function deletes nothing until something calls it, so `docs/TELEMETRY.md`'s 90/90/30 table is a rule the database knows and does not apply. Both invocations — a `pg_cron` entry and an external caller — are written out ready to run in that migration's section 7, with which is the owner's to pick and why. `pg_cron` additionally has to be enabled per project from the Supabase dashboard, and a `create extension` that fails because it is unavailable would fail the whole migration, which is why the extension is not in the file.

**And the client-side pair is unchanged**, including the sentence above about setting the Worker binding first.
...
- **This document's Contract paragraph survives untouched.** It says *"a server-side Worker entry point must not be added merely to serve the SPA"*. An ingest handler is not that, and the sentence needs no edit. (The *first* sentence of that paragraph did change, on 2026-09-03: it said "assets-only". This bullet was about the last sentence and is still right about it.)
...
**Discharged 2026-09-03.** The change landed and ADR 0002 carries *"Amendment, 2026-09-03: the assets-only bullet is now false, and this is the commit it went false in"*, which does exactly what the paragraph above specified: it quotes the Decision bullet, leaves it standing, states in its own opening that the decision is the owner's and the wording is not, and touches no status. Two things it adds that this section did not predict: `assets.run_worker_first` had to become `true` for a Worker route to be reachable beside SPA fallback at all, and the `public/_headers` bullet needed no edit but does not reach a response the Worker writes itself.
