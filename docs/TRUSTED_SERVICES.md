...
- `entitlements` and `entitlement_events`: SELECT-own policies only, and
  the default INSERT/UPDATE/DELETE grants are revoked outright. Both halves
  of each of those two policies are now asserted — that a different account
  reads none of them, and that the owner reads their own. The second half
  was missing until `supabase/tests/009_rls_policy_surface.test.sql`, and it
  is not a formality either: `entitlement_events_select_own` exists so a
  player can see *why* they hold a capacity ("an entitlement a player cannot
  see the reason for is indistinguishable from one we invented"), and
  dropping it entirely left the whole suite green, because a cross-account
  read returns zero rows whether the policy scopes the table or denies it.
