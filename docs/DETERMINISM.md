...
- `setControl` pumps before it swaps, so wall time that passed under the *old* control is accounted for under that control. A paused clock therefore discards the time it was paused for instead of banking it and spending it as a burst on resume.
