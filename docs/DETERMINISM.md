...
- A registry-shaped `all()` sorts. `ContentRegistry`, `SecuritySectorRegistry`, `GangRegistry` and — since #132 — `DoorRegistry` all do; an accessor that hands out registration history is a trap for its next caller even where today's only caller sorts what it gets. Where an accessor deliberately does not (`RoomInstanceRegistry.occupantsOf`), that is stated at the accessor and its consumer sorts, and the arrangement has its own test. The one *simulation* consumer of a room's occupants does not use that accessor at all: `RoomInstanceRegistry.findBestAvailable` sorts ascending by entity id before handing them to its rating function, because insertion order there is assignment order live and ascending-id order after a restore, and the same run would place a prisoner differently on the two sides of a save.
