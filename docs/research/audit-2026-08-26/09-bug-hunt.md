...
| `JobWorkerPool.unregister` (`job-system.ts:38-41`) | `workers`, `busy` | ✗ jobs already assigned to that worker keep running and call `workerAdapter.getPositionTile(deadId)` (throws). No caller in `src/` today — latent |
