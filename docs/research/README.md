...
| [2026-08-29 mouse playtest](./2026-08-29-mouse-playtest.md) | Playing with the mouse only, where does a new player stop being told what the game wants? | **[#569](https://github.com/matmaxalez/lockstate/issues/569)** — a refused designation's explanation is rendered into a section that starts folded and carries no badge, so the whole refusal channel is invisible by default. The reproduction stays unmerged on `agent/playtest-mouse-route`; it is a harness, not a gate |
