...
| [2026-08-29 dismissing a staff member](./2026-08-29-dismissing-a-staff-member-adr-draft.md) | How does a prison let a staff member go, and what does a sector with nobody in it ask for? | None yet — it is an **ADR draft awaiting a centrally assigned number**, filed here because `tests/foundation/adr-numbering-contract.test.ts` admits no unnumbered draft in `docs/adr/`. It amends ADR 0048 decision 3 and answers the dismissal half of ADR 0033 open question 3 that ADR 0034 deliberately left |
