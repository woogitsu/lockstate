...
| Browser E2E | Real browser storage/durability, real `DOMException` names, real quota exhaustion, save migration off real storage, whether a real DOM lifecycle event reaches its handler, and the assembled page: canvas sizing, hit-testing, image decode | `tests/browser/`, via `pnpm test:browser` (own command, required CI job) |
