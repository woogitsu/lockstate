...
export class EntityQuery {
   ...
   * order only while every live slot shares a generation. Today it always
   * does, because nothing in `src/` destroys an entity (#31), so no index is
   * ever recycled; the moment a release path exists, a recycled slot at a low
   * index sorts *after* a fresh slot at a higher one. Measured:
   * `getIdByIndex(0)` at generation 2,048 is 2,147,483,648 while
   * `getIdByIndex(1)` at generation 0 is 1.
   *
