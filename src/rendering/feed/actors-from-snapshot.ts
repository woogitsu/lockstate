...
/**
 * The renderer's view of the prisoners a session snapshot carries.
 *
 ...
 *
 * Guards are the other population whose tiles the bundle carries
 * (`simulation.security.guards`, as `GuardRecord.tileX`/`tileY`). They are
 * deliberately not decoded here: that is a separate step with its own asset
 * choice, and this one is scoped to prisoners.
 *
 * ### Cost
 *
 ...
 * object per *live* prisoner. `decodeEntityStoreSnapshot` additionally
...
export function actorsFromSnapshot(
  ...
  for (let index = 0; index <= lastIndex; index += 1) {
    ...
    actors.push({
      ...
      id: packEntityId(index, store.generations[index]!),
  ...
  // Ascending entity index: the canonical order `EntityQuery.execute` walks
  // (ADR 0005), so the array is the same on every client and after a
