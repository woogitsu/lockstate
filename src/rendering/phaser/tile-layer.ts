...
import { edgeArt, objectSprite, terrainFloorSpriteAt, zonedFloorSprite } from '../world/environment-art';
      ...
      for (let localX = 0; localX <= size; localX += 1) {
        ...
        if (localX < size) {
          ...
          const candidate = art === undefined ? undefined :
            (zonedFloorSprite(sample.zoning) ?? terrainFloorSpriteAt(sample.terrainNumericId, originTileX + localX, tileY));
