...
/**
 * The Prisoners panel: who is in the prison, and what the chosen one of them
 * needs (issue #451, split out of the Regime panel for ADR 0115).
 *
 * ### Why this is its own module, and what the split is and is not
 *
 * Until this change the roster, the inspector and the day's timetable were one
 * `createPanel` in `regime-panel.ts`, and that file's own header argued for it:
 * *"The two blocks are one subject read at two scales"*. That argument is
 * untouched -- **both panels are mounted on the Plan dnia tab and are laid out
 * together**, so the cause and effect it names is still on screen at once.
 * What changed is that the roster is now a panel rather than a block, which
 * buys three things a block cannot have:
 *
 * 1. **Its own collapse.** `createPanel`'s chevron folded both halves
 *    together; a player can now fold either one and keep the other, which is
 *    the only way to give the roster the whole rail on a short viewport
 *    without a layout rule deciding it for them.
 * 2. **Its own height budget.** `.ui-panel.hud-roster` is `overflow-y: auto`
 *    and `.ui-panel.hud-regime` is `flex: 0 0 auto`, which is the division of
 *    labour `hud.css` records for `.ui-panel.hud-staff` and
 *    `.ui-panel.hud-intake` on the Manage tab: the panel whose height grows
 *    with the population absorbs a short rail, and the panel bounded by a
 *    closed catalogue keeps its natural height. One panel could not make that
 *    distinction about its own two halves at all.
 * 3. **Its own mount.** ADR 0115's option 4 is explicitly the shape that
 *    leaves the placement one decision away -- *"let the placement be a
 *    one-line mount change afterwards"*. That is true only of a panel: this
 *    panel's `element` is appended to `.hud__side` and shown by one
 *    `setVisible` call in `hud.ts`, exactly as the Intake panel's was when the
 *    owner's ruling of 2026-09-14 moved it between tabs.
 *
 * **What it does not buy is an independent scroll, and that is worth stating
 * because it is the first thing a reader will assume it bought.** The obvious
 * argument -- one scroll box means reaching the last roster row drags the
 * timetable out of sight -- describes something that never happened:
 * measured on `a3a9e7f3` with the browser suite's fullest fixture, the single
 * panel's `scrollHeight - clientHeight` was **0 at all five viewports**, so
 * there was no scroll for the timetable to be carried by. The property is now
 * structural rather than incidental, which is worth having and is not a
 * repair.
 *
 * **What it does not do is move anything.** The owner's ruling of 2026-09-16
 * on ADR 0115 chose *"rozbij panel w kodzie, obie połowy na Plan dnia"* --
 * split the panel in code, both halves on Plan dnia -- and declined the option
 * that put the roster on Manage. Nothing here may be read as preparing that
 * move: it is ruled, not deferred, and moving this panel to another tab is a
 * fresh decision for the owner rather than a one-line change an agent may
 * spend.
 *
 * **What the split costs a player is one panel header -- 63px of chrome, and
 * an arrival scroll at two of five viewports** -- and it is stated here
 * rather than left to be discovered, because this HUD's rail has been fixed
 * twice for exactly this kind of arithmetic (#143, #174). ADR 0115 predicted
 * the direction of it in its own "weakest claim" section: *"A roster panel of
 * its own carries panel chrome this measurement does not include ... the
 * direction of the error is known."* The table is in `hud.css`'s
 * `.ui-panel.hud-roster` block, one panel read on `a3a9e7f3` and two on this
 * tree. Nothing became unreachable, which is stage 5's exit criterion; what
 * changed is that at 375x812 and 900x600 the panel arrives 16px and 46px
 * scrolled to show the inspector, where one panel arrived unscrolled.
 *
 * ### Boundaries
 *
 * A composer, exactly like the Staff and Intake panels. It imports nothing
 * from `src/simulation/**`, and every figure and every word it renders arrives
 * on the view model as a number or a message key. In particular it does
 * **not** count anything: the roster's `total` is the projection's, and the
 * window is whatever the host asked for -- a panel that counted its own rows
 * would report the window as the population.
 *
 * It holds one selection -- which prisoner the inspector is about -- and
 * issues one intent to say so. That selection is **chrome**: applied here
 * immediately and unconditionally, with the host merely told, because the host
 * is the only thing that can turn a chosen prisoner into a
 * `hud/prisoner-detail` request. Nothing here decides simulation state and
 * nothing here counts.
 *
 ...
 * still not a date.
 *
 * **#958 correction:** the inspector adds a sentence-remaining readout from its
 * own projection tick and the existing clock's day length.
 *
 * **So the inspector adds no player-facing sentence of its own.** Its heading
 * is the prisoner's *name*, which is state rather than copy (ADR 0015), its
 * badge is the same word the row's badge carries, and its six lines are the six
 * need words `simulation-message-keys.ts` authored. The one thing it cannot say
 * without the owner is why the block has gone away when a selected prisoner is
 * released; it therefore says nothing, and the block leaves rather than
 * standing there with a placeholder in it.
 *
 * ### The need bar, and the decision it reverses
 *
 * Each row draws that prisoner's **worst** need as a bar (issue #535,
 * decision 6). The block this replaced was headed "What it deliberately does
 * not show" and refused exactly that, in these words:
 *
 ...
 * (`docs/AGENT_WORKFLOW.md` §4: mark both directions). The premise expired at
 * `d1b6b5c` (#488), which added `STATE_INCOME_UNMET_NEED_LEVEL` -- the level at
 * or below which the state withholds part of that prisoner's day of the
 * operating grant. `PrisonerNeedViewModel.unmetForStateIncome` is that
 * projected threshold; the panel compares nothing and reads a flag, so this is
 * the refusal's own condition being met rather than the refusal being
 * overridden.
 *
 * **What the tone means did not move when the rate did.** The owner suspended
 * the withheld share at `0` on 2026-09-03 and restored it to `40` on
 * 2026-09-04, and not a line of this readout changed on either date, because
 * the tone means *this need is at or below `STATE_INCOME_UNMET_NEED_LEVEL`* --
 * the line the state reads when it settles a day, whatever it currently charges
 * for crossing it.
 *
 * `describeNeed` is the single place that flag becomes a colour; every other
 * reader copies it out as data for a probe and decides nothing.
 *
 ...
 * a word plus a meter, and the prisoner's standing as a pill. Their
 * vocabularies overlap -- `Low` is a risk tier in the pill and, read as the
 * meter's grade, the opposite instruction -- and the playtest that found it
 * misread its own sample twice
 * (`docs/research/2026-09-03-can-a-player-read-this.md` §4). The fix is
 * composition rather than copy: the pill is on the **name's** line, where the
 * inspector has always drawn it, and the meter prints its own percentage so the
 * two scales differ in kind as well as in position. `paintRoster`'s row builder
 * carries the reasoning, the rejected alternatives and the before-and-after
 * geometry; `tests/browser/ui-roster-row-scales.spec.ts` is the gate.
 *
 ...
 *
 * **THE LAST SENTENCE EXPIRED ON 2026-09-16 AND THE TABLE ABOVE IS TWO
 * MEASUREMENTS OLD. Both are kept rather than overwritten
 * (`docs/AGENT_WORKFLOW.md` §4), because what they predicted is exactly what
 * happened and by the amount they predicted.** #958 added the
 * sentence-remaining line this block said would cost it, and ADR 0115's split
 * added the second panel header. Re-read from the same test on this tree:
 *
 * | viewport | block | panel fold | overflow | scrollTop |
 * | --- | --- | --- | --- | --- |
 * | 1280x720 | y=601.0..699.0 | 706.6 | 0.0 | 0.0 |
 * | 1440x900 | y=781.0..879.0 | 886.6 | 0.0 | 0.0 |
 * | 1024x768 | y=649.0..747.0 | 754.6 | 0.0 | 0.0 |
 * | 900x600 | y=494.0..592.0 | 591.4 | **46.0** | **38.0** |
 * | 375x812 | y=620.0..718.0 | 717.4 | **16.0** | **8.0** |
 *
 * The block is **98.0px** rather than 81.0, and `revealDetail` now fires at
 * the two short viewports rather than at none: the panel scrolls 38px and 8px
 * on the press that selects a prisoner, which is the affordance that paragraph
 * said would have to carry it. Nothing is unreachable -- the inspector's *top*
 * is inside the fold at all five, which is what the test asserts -- and the
 * arrival state with nobody selected still overflows by 0 everywhere.
 */
...
export function createRosterPanel(options: RosterPanelOptions): RosterPanel {
  ...
  const rosterBlock = element('div', {
    ...
    children: [
      /*
       * The count, and no longer the word beside it.
       *
       * `hud.regime.roster` used to be an eyebrow on this line with the count
       * after it; it is the **panel's** title now, one level up, because a
       * panel headed "Prisoners" whose first line also reads "Prisoners" says
       * the same noun twice in a rail this HUD has been fixed twice for
       * overfilling (#143, #174). The key is the same key and the word a
       * player reads is the same word, so this split authors no
       * player-visible string and retires none -- it moves one.
       *
       * The row itself survives rather than the count moving into the panel
       * header, because `.hud-regime__roster-count` is what `regimeProbe`
       * reads for "N of M" and what the empty-prison sentence sits under.
       */
      element('div', {
        className: 'hud-regime__roster-header',
        children: [rosterCount],
      }),
      rosterList,
      rosterEmpty,
      rosterMore,
  ...
  let collapsed = false;
  /*
   * **The panel's title is `hud.regime.roster`, which is the word the roster
   * block's own header used to carry**, so this split authors no new
   * player-visible string: "Prisoners" moved up one level from an eyebrow
   * inside the panel to the panel's heading, and the count that shared that
   * line with it stayed where it was. Reusing the key rather than adding one
   * is also what keeps the two in step -- a second noun for the same list
   * would be a second vocabulary for it, which is the trap
   * `editorGroupFor`'s legend note one panel over names.
   *
   * The icon is `prisoners`, the one the tab bar already spends on this
   * subject, rather than `regime` -- two panels on one tab carrying the same
   * glyph would say they are the same readout.
   */
  const panel = createPanel({
    title: t(HUD_MESSAGE_KEY.regimeRoster),
    icon: 'prisoners',
    className: 'hud-roster',
    collapse: {
  ...
  /*
   * The inspector goes below the roster, and that is reading order rather than
   * an aesthetic: a player picks a prisoner from the list and then reads about
   * them, so the answer sits under the question. It is also the only place it
   * can go without re-measuring what is already measured -- the roster's four
   * rows and its "and N more" line are asserted to sit inside this panel's fold
   * at all five viewports the browser suite visits
   ...
   *
   * Two blocks in this body and not one nested in the other, so that
   ...
   * the lowest of the rows, the "and N more" line and that sentence. A need
   * line nested in there would join both of those without meaning to.
   */
  panel.body.append(rosterBlock, detailBlock);
